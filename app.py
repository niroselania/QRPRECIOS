...
def _get_price_lookup(path):
    """Lee el precio de venta (RETAIL) del STOCK y lo agrupa por SKU + color. Ese valor es sin IVA."""
    global _PRICE_CACHE
    mtime = os.path.getmtime(path)
    ...
    for row in filas:
        if row is None or len(row) <= max(col_sku, col_color, col_precio):
            continue
        key = _clave_producto(row[col_sku], row[col_color])
...
def generar_etiqueta(descripcion, color, talle, codigo, precio_sin_iva, sku="", dpi=300):
    ...
    y += round(1.55 * px_mm)
    draw.text((margen, y), _money(round(precio_sin_iva * IVA)), font=font_price, fill=0)
...
def etiqueta_generar():
    ...
    if not barcode_scanned:
        return _error_etiqueta("Escaneá o pegá un código de barras.")

    grande_path = _resolver_grande_cache()
    if not grande_path:
        return _error_etiqueta("No hay planilla de equivalencia cargada. Subila primero.")

    try:
        producto = _get_barcode_index(grande_path).get(_clave_barra(barcode_scanned))
    except Exception as e:
        return _error_etiqueta(f"Error buscando el producto: {e}", 500)

    if not producto:
        return _error_etiqueta(f"No encontré el código {barcode_scanned} en la planilla de equivalencia.")

    stock_path = _resolver_stock_cache()
    if not stock_path:
        return _error_etiqueta("No hay planilla de STOCK con precios cargada. Subila primero.")

    try:
        precio_sin_iva = _get_price_lookup(stock_path).get(_clave_producto(producto["sku"], producto["color"]))
    except Exception as e:
        return _error_etiqueta(f"Error leyendo los precios: {e}", 500)
    if precio_sin_iva is None:
        return _error_etiqueta(f"No encontré precio para {producto['sku']} {producto['color']} en la planilla STOCK.")
